...
while (m < nrow(STK)) {
  if ( # close crossover ma1 & ma1>ma2
    ...
    A[m -1] <- as.numeric(A[m - 2]) + (as.numeric(cl[m]) - long) * share + 
...
# 
## Add Legend
legend("bottomleft",legend=c("PL","Asset"),
       text.col=c("red","orange"), lwd=3,col=c("red","orange"), seg.len=0.5,
       cex=0.75)


# axis(1, at=format(seq(as.Date("2012-01-01"), as.Date("2022-12-30"), by="months"), format="%Y-%m"))
# axis(1, seq(as.Date("2012-01-01"), as.Date("2022-12-30"), by="months"), format="%m-%Y")
axis(1, 1:length(PL)/365, as.Date("2012-01-01") + 365 * (1:length(PL)/365))
