...
ma=5
ma2=30
row=Cl(STK)>WMA(Cl(STK),ma) # 收盤 均線1
row2=WMA(Cl(STK),ma)>WMA(Cl(STK),ma2) # 均線1 均線2
#View(cbind(Cl(STK),SMA(Cl(STK),ma),SMA(Cl(STK),ma2),row,row2))
(PL=setNames(rep(0,length(Cl(STK))),time(STK)))
m=ma2+1
count=0
while ( m < nrow(STK)){
  if ((row[m-1]==0 && row[m]==1) || (row2[m-1]==0 && row2[m]==1 )){ # cl>ma1 | ma1>ma2
    long=as.numeric(Op(STK)[m+1]) # 進場開盤價
    count=count+1
    while( (row[m]==1 || row2[m]==1) && m <nrow(STK)-1){m=m+1} # cl>ma | ma1>ma2
...
plot(cumsum(PL), type="l", lwd=2, col="blue", ylim = Yrange
